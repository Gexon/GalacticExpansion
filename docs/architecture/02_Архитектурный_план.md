...
graph TB
    ...
    subgraph GLEXMod [GLEX Mod DLL]
        ...
        TransportMgr[Transport Manager<br/>Unit Transportation]
    end
    ...
    TransportMgr --> Gateway
    TransportMgr --> SpawnEvo
    TransportMgr --> UnitEconomy
...
#### 3.2.13 Transport Manager

**Назначение:** Управление транспортировкой юнитов внутри планеты

**Ответственность:**
- Доставка защитников к месту атаки через транспорт
- Умный выбор метода доставки (транспорт vs прямой спавн)
- Запуск патрульных кораблей из ангара базы
- Логистические рейсы между базой и аванпостами

**Технологии:** C#, integration с Entity Spawner и AIM Orchestrator

**Паттерны:** Strategy (выбор метода доставки), State Machine (фазы миссии)

**Интерфейсы:**
```csharp
public interface ITransportManager
{
    // Умная доставка защитников (автовыбор метода)
    Task<TransportMission> DeployDefendersAsync(Colony colony, Vector3 destination, int count);
    
    // Запуск патрульного корабля
    Task<int> LaunchPatrolVesselAsync(Colony colony, List<Vector3> patrolRoute);
    
    // Логистические рейсы (визуальный эффект)
    Task StartLogisticsRunAsync(Colony colony, Vector3 outpostPosition);
    
    // Обновление активных миссий
...
public class TransportMission
{
    ...
    public bool IsDirectSpawn { get; set; }  // true = без транспорта (база/портал)
}

public enum TransportType { None, DropShip, PatrolSV, LogisticsHV, LogisticsSV }
```

**Ключевые особенности:**
- **Локальная логика:** Транспорт только внутри планеты (межсистемные перелеты НЕ реализуются)
- **Умный выбор:** Атака на базу → спавн из базы/портала, атака на аванпост → транспорт
- **Иммерсивность:** Игрок видит десантные корабли, летящие от базы к месту инцидента
- **Тактическая глубина:** Игрок может перехватить транспорт до высадки юнитов
